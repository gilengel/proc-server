...
export function buildParameterPin(
  ...
  if (direction === Direction.Out) {
    const pin = new Rete.Output(
      parameter.id !== undefined ? parameter.id : parameter.type,
      parameter.label,
      socket,
    );
    node.addOutput(pin);
    ...
    if (parameter.control !== undefined) {
      node.addControl(
        createControl(
          ...
          editor,
          node,
          parameter.control.identifier !== undefined
            ? parameter.control.identifier
            : parameter.type,
          parameter.value,
          // eslint-disable-next-line @typescript-eslint/unbound-method
          parameter.control.isValid !== undefined
            ? parameter.control.isValid
            : () => true,
        ),
      );
    }
...
export class FlowComponent extends Rete.Component {
  ...
  builder(node: ReteNode): Promise<void> {
    ...
    if (!editor) {
      throw new Error(
        `Cannot create instance of type FlowComponent because the provided editor is undefined`,
      );
    }

    if (this.schema.inputs) {
      this.schema.inputs.forEach((parameter) => {
        buildParameterPin(editor, node, parameter, Direction.In);
      });
    }

    if (this.schema.outputs) {
      this.schema.outputs.forEach((parameter) => {
        buildParameterPin(editor, node, parameter, Direction.Out);
      });
    }
